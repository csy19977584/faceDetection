...
def center_loss(labels, features, alpha=ALPHA, num_classes = NUM_CLASSES):
    ...
    global i
    print("KKKKKKKKKKKKKKKKKKKKKKKKKKKK:",i)
    centers  = tf.get_variable('centers' + str(i), [num_classes, len_features], dtype=tf.float32,
...
def softmax_loss(labels, features):
    ...
    """
    print("GGGGGGGGGGGGGlabels:",labels.shape)
...
def loss(labels, features):
    ...
    """
    print("yyyyyyyyyyyfeatures:",features.shape)
    labels = K.cast(labels, dtype=tf.float32)
    print("XXXXXXXXXXXlabels:",labels.shape)
    # 计算softmax-loss
